...
namespace vks
{
	...
	void UIOverlay::prepareResources()
	{
		...
		io.Fonts->AddFontFromFileTTF(filePath.generic_string().c_str(), 16.0f * scale);
		...
		VK_CHECK_RESULT(vkCreateImage(device->mLogicalVkDevice, &imageInfo, nullptr, &fontImage));
		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(device->mLogicalVkDevice, fontImage, &memReqs);
		VkMemoryAllocateInfo memAllocInfo{
			.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
			.allocationSize = memReqs.size,
			.memoryTypeIndex = device->GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
		};
		VK_CHECK_RESULT(vkAllocateMemory(device->mLogicalVkDevice, &memAllocInfo, nullptr, &fontMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device->mLogicalVkDevice, fontImage, fontMemory, 0));
		...
		VK_CHECK_RESULT(vkCreateImageView(device->mLogicalVkDevice, &viewInfo, nullptr, &fontView));
		...
		vkCmdCopyBufferToImage(copyCmd, stagingBuffer.mVkBuffer, fontImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
		...
		VK_CHECK_RESULT(vkCreateSampler(device->mLogicalVkDevice, &samplerInfo, nullptr, &sampler));
		...
		VK_CHECK_RESULT(vkCreateDescriptorPool(device->mLogicalVkDevice, &descriptorPoolInfo, nullptr, &descriptorPool));
		...
		VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->mLogicalVkDevice, &descriptorLayout, nullptr, &descriptorSetLayout));

		// Descriptor set
		VkDescriptorSetAllocateInfo allocInfo = VulkanInitializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device->mLogicalVkDevice, &allocInfo, &descriptorSet));
		...
		VkWriteDescriptorSet writeDescriptorSets = VulkanInitializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &fontDescriptor);
		vkUpdateDescriptorSets(device->mLogicalVkDevice, 1, &writeDescriptorSets, 0, nullptr);
	...
	void UIOverlay::preparePipeline(const VkPipelineCache pipelineCache, const VkRenderPass renderPass, const VkFormat colorFormat, const VkFormat depthFormat)
	{
		...
		VK_CHECK_RESULT(vkCreatePipelineLayout(device->mLogicalVkDevice, &pipelineLayoutCreateInfo, nullptr, &pipelineLayout));
		...
		VkPipelineMultisampleStateCreateInfo multisampleState = VulkanInitializers::pipelineMultisampleStateCreateInfo(rasterizationSamples);
		...
		VkGraphicsPipelineCreateInfo pipelineCreateInfo{
			...
			.stageCount = static_cast<std::uint32_t>(shaders.size()),
...
#endif
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device->mLogicalVkDevice, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
	}

	/** Update vertex and index buffer containing the imGui elements when required */
	void UIOverlay::update(std::uint32_t currentBuffer)
	{
		...
		if ((buffers[currentBuffer].vertexBuffer.mVkBuffer == VK_NULL_HANDLE) || (buffers[currentBuffer].vertexBuffer.mVkDeviceSize < vertexBufferSize))
		{
		...
		if ((buffers[currentBuffer].indexBuffer.mVkBuffer == VK_NULL_HANDLE) || (buffers[currentBuffer].indexBuffer.mVkDeviceSize < indexBufferSize))
		{
		...
		// Upload data
		ImDrawVert* vtxDst = (ImDrawVert*)buffers[currentBuffer].vertexBuffer.mMappedData;
		ImDrawIdx* idxDst = (ImDrawIdx*)buffers[currentBuffer].indexBuffer.mMappedData;
	...
	void UIOverlay::draw(const VkCommandBuffer commandBuffer, std::uint32_t currentBuffer)
	{
		ImDrawData* imDrawData = ImGui::GetDrawData();
		...
		if (buffers[currentBuffer].vertexBuffer.mVkBuffer == VK_NULL_HANDLE || buffers[currentBuffer].indexBuffer.mVkBuffer == VK_NULL_HANDLE)
		{
			return;
		}
		...
		assert(buffers[currentBuffer].vertexBuffer.mVkBuffer != VK_NULL_HANDLE && buffers[currentBuffer].indexBuffer.mVkBuffer != VK_NULL_HANDLE);

		VkDeviceSize offsets[1] = {0};
		vkCmdBindVertexBuffers(commandBuffer, 0, 1, &buffers[currentBuffer].vertexBuffer.mVkBuffer, offsets);
		vkCmdBindIndexBuffer(commandBuffer, buffers[currentBuffer].indexBuffer.mVkBuffer, 0, VK_INDEX_TYPE_UINT16);
	...
	void UIOverlay::resize(std::uint32_t width, std::uint32_t height)
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void UIOverlay::freeResources()
	{
		...
		vkDestroyImageView(device->mLogicalVkDevice, fontView, nullptr);
		vkDestroyImage(device->mLogicalVkDevice, fontImage, nullptr);
		vkFreeMemory(device->mLogicalVkDevice, fontMemory, nullptr);
		vkDestroySampler(device->mLogicalVkDevice, sampler, nullptr);
		vkDestroyDescriptorSetLayout(device->mLogicalVkDevice, descriptorSetLayout, nullptr);
		vkDestroyDescriptorPool(device->mLogicalVkDevice, descriptorPool, nullptr);
		vkDestroyPipelineLayout(device->mLogicalVkDevice, pipelineLayout, nullptr);
		vkDestroyPipeline(device->mLogicalVkDevice, pipeline, nullptr);
	}
	...
	bool UIOverlay::checkBox(const char* caption, std::int32_t* value)
	{
		bool val = (*value == 1);
	...
	bool UIOverlay::sliderInt(const char* caption, std::int32_t* value, std::int32_t min, std::int32_t max)
	{
		return ImGui::SliderInt(caption, value, min, max);
	}

	bool UIOverlay::comboBox(const char* caption, std::int32_t* itemindex, std::vector<std::string> items)
	{
		if (items.empty())
		{
			return false;
